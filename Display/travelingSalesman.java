...
import GeneticObjects.CityRoute;
...
public class travelingSalesman extends GraphicsProgram{
	...
	// Will store the candidate solution
	public static ArrayList<City> candidate = new ArrayList<City>();
	...
	private void plotCities()throws FileNotFoundException {
		...
		for (int i=0; i<=citiesToTravel; i++ ) {
			...
			//add new Points to ArrayList of type point to our candidate solution route
			//candidate.add(new Point(x,y));
			
			//candidate.add(new City(x,y));
			CityRoute.addCity(new City(x,y));
